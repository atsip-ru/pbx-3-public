...
function preinstall() {
    ...
    sudo apt install iputils-ping nmap net-tools tcpdump sngrep rinetd dnsutils -y
    echo -e "${color}Дополнительные утилиты:${nocolor}"
    sudo apt install sox mpg123 cron mc openvpn fail2ban tftpd-hpa git htop iptables ntp -y
}
...
function inst_scripts(){
    ...
    # Скрипты автоудаления старых записей и голосовой почты
...
function customize(){
    # Добавляю своё лого
    sed -i 's/BRAND_IMAGE_SPONSOR_FOOT=images\/sangoma-horizontal_thumb.png/BRAND_IMAGE_SPONSOR_FOOT=images\/atsip.ru.png/' /etc/amportal.conf
    # Вот тут неясно что не нравится
    #sed -i 's/BRAND_SPONSOR_ALT_FOOT=www.sangoma.com/BRAND_SPONSOR_ALT_FOOT=https://atsip-ru.github.io/' /etc/amportal.conf
    # Уменьшаем генерирование пароля до 8 символов
    sed -i 's/SIPSECRETSIZE=32/SIPSECRETSIZE=8/' /etc/amportal.conf
    # Отображение хостнейма в строке браузера
    sed -i 's/SERVERINTITLE=FALSE/SERVERINTITLE=TRUE/' /etc/amportal.conf
...
#\033[33m Донастрой АТС по адресу http://${IP_ADDR}/pbx/   \033[34m#
########################################################\033[0m"
